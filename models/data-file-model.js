...
exports.insertDB = (inputFiles, request, response) => {
  ...
  let strSQL = `INSERT INTO datafiles
...
exports.getAll = result => {
  strSQL = `select * from datafiles order by file_id desc`;
...
exports.findById = (fileId, result) => {
  sqlConnection.query(`SELECT * FROM datafiles WHERE file_id = ${fileId}`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
exports.updateById = (id, reqbody, result) => {
  ...
  sqlConnection.query(
    "UPDATE datafiles SET status = ? WHERE file_id = ?",
    [reqbody.status, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
